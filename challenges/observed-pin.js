...
function getPINs(observed) {
  ...
  const pos = [[0,8],[1,2,4],[1,2,3,5],[2,3,6],[1,4,5,7],[2,4,5,6,8],[3,5,6,9],[4,7,8],[0,5,7,8,9],[6,8,9]];
  const combos = [];
  const results = [];
  let entryArr = observed.split('');
  console.log(entryArr);
  entryArr.forEach(num => combos.push(pos[+num]));
  console.log(combos)
  // for (let j = 0; j < combos.length; j += 1) {

  // }
  combos.forEach((group, i) => {
    group.forEach(num => results.push(String(num)))
  })
  console.log(results)
}
