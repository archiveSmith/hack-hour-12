...
function getPINs(observed) {
  ...
  function getVariations(observed, variation = '') {
    if (observed === '') {
      variations.push(variation);
      return;
    }
    for (var i = 0; i < possibleNums[observed[0]].length; i++) {
      getVariations(observed.slice(1), variation + possibleNums[observed[0]][i]);
    }
  }
  getVariations(observed);
  return variations;
}
